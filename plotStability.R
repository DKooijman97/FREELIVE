...
#' @export
plotStability=function(MVObject,model='min',VAll,nVarLim,missLim,Variable_selection = "vip" ) {
  ...
  if(missing(VAll)) {
    if (Variable_selection == "Sratio"){
       VAll=names(sort(MVObject$Sratio[,nModel])[1:nVar]) # Final selection of variables
    }  
    else {
      VAll=names(sort(MVObject$VIP[,nModel])[1:nVar]) # Final selection of variables
    }
  }
  ...
  for (i in 1:nRep) {
    ...
    if (Variable_selection == "Sratio"){ 
      VARep[i]=sum(names(sort(MVObject$SratioPerRep[[nModel]][,i])[1:nVRep[i]])%in%VAll)
      VA[i]=sum(names(sort(rowMeans(MVObject$SratioPerRep[[nModel]][,1:i,drop=F]))[1:nV[i]])%in%VAll)
    }
    else{
