...
#' @param Variable_selection Option to choose the variable selection method
#' 
...
#' @export
#'
plsInner=function(xTrain,yTrain,xVal,yVal,DA,fitness,comp,scale=TRUE, Variable_selection = c("vip","Sratio")) {
  ...
  if (comp>0){
    ...
    if (Variable_selection == "vip") {
       returnIn$vi=rank(-vip(plsModIn)[,nComp])
    }
    
    else {
      
      returnIn$vi=rank(-Sratio(plsModIn)[,1])         #DennisK
