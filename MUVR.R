#' Modified MUVR-module: "Multivariate modelling with Unbiased Variable selection"
#' Wrapper for the MUVR program to include Sratio as variable selection method
#' 
#' Additional Sratio.R & plsInner.R files needed in working directory for Sratio to work.
#' 
#' Original MUVR module downloaded (on 6-5-2019) from: https://gitlab.com/CarlBrunius/MUVR
#' 
#' MUVR-module modified by: Dennis Kooijman
...
#' @param Variable_selection Option to choose the variable selection method, defaults to VIP
#' @return A MUVR object
#' @export
MUVR=function(X,Y,ID,scale=TRUE,nRep=5,nOuter=6,nInner,varRatio=0.75,DA=FALSE,fitness=c('AUROC','MISS','BER','RMSEP'),method=c('PLS','RF'), nCompMax,methParam,ML=FALSE,modReturn=FALSE,logg=FALSE,parallel=TRUE, Variable_selection = c('vip','Sratio')){
  library(pROC)
  library(foreach)
  print("start MUVR")
  if (parallel) "%doVersion%"=get("%dopar%") else "%doVersion%"=get("%do%")
  if (missing(method)) method='RF'
  if (missing(Variable_selection)) Variable_selection = "vip"
  ...
  InData=list(X=X,Y=Y,ID=ID,scale=scale,nRep=nRep,nOuter=nOuter,nInner=nInner,varRatio=varRatio,DA=DA,fitness=fitness,method=method,methParam=methParam,ML=ML,parallel=parallel, Variable_selection = Variable_selection)
  ...
  reps=foreach(r=1:nRep, .packages=packs, .export=exports) %doVersion% {
    ...
    for (i in 1:nOuter) {   
      ...
      for (count in 1:cnt) {  # Build models with successively fewer variables. Quality metric = number of missclassifications for Validation set
        ...
        for (j in 1:nInner) {
          ...
          if (method=='PLS') {
            ...
            inMod=plsInner(xTrain,yTrain,xVal,yVal,DA,fitness,comp,scale=scale, Variable_selection = Variable_selection)
            
            nCompIn[j,count]=inMod$nComp #count is number of variables, j in number of reps
          } else {
          ...
          # Store VIs
          VIPInner[match(names(inMod$vi),rownames(VIPInner)),count,j]=inMod$vi   #DennisK
        }
      ...
      #stop("before avering inner loop")
    ...
    #stop("before Average outer loop predictions" )
  ...
  if (Variable_selection == "Sratio") {
    modelReturn$Sratio=VIP
    modelReturn$SratioPerRep=list(minModel=VIPRepMin,midModel=VIPRepMid,maxModel=VIPRepMax)
  }
  else {modelReturn$VIP=VIP
